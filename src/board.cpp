...
bool Board::load_from_file(const std::string& filename) {
    ...
    auto parse_metadata = [&](const std::string& expected_key, size_t& target) -> bool {
        std::getline(file, line);
        auto pos = line.find("=");
    ...
    if (!parse_metadata("MaxSteps", max_steps)) return false;
    if (!parse_metadata("NumShells", num_shells)) return false;
    if (!parse_metadata("Rows", height_)) return false;
    if (!parse_metadata("Cols", width_)) return false;
...
bool Board::execute_tank_action(std::shared_ptr<Tank> tank, TankAction action) {
    ...
    switch (action) {
        ...
        case TankAction::Shoot: {
            if constexpr (config::get<bool>("verbose_debug")) std::cout << "[Board] Executing Shoot for Tank " << tank->id() << std::endl;
        ...
        case TankAction::Idle: {
            if constexpr (config::get<bool>("verbose_debug")) std::cout << "[Board] Executing Idle for Tank " << tank->id() << std::endl;
...
void Board::update_active_shells() {
    ...
    for (auto& [from, to, shell] : moves) {
        ...
        if (shells_to_remove.count(shell) == 0) {
            ...
            auto it = std::find_if(active_shells_.begin(), active_shells_.end(), [&](const auto& p) { return p.second == shell; });
...
void Board::resolve_collisions(Cell& cell) {
    if ((((cell.has(ObjectType::Shell) && !cell.has(ObjectType::Mine)) || (!cell.has(ObjectType::Shell) && cell.has(ObjectType::Mine))) &&
