...
TEST_F(BoardTest, TankCannotPassThroughWall) {
    ...
    tank->position() = std::make_pair(0, 5);
    board.grid()[0][5] = Cell({0, 5}, tank);
    ...
    EXPECT_EQ(tank->position(), Position(0, 5));  // Didn't move
}
...
TEST_F(BoardTest, TankShootsAndHitsOtherTank) {
    ...
    tank1->position() = std::make_pair(5, 4);
    tank2->position() = std::make_pair(5, 6);
    tank1->direction() = Direction::R;
    tank2->direction() = Direction::L;

    board.grid()[5][4] = Cell({5, 4}, tank1);
    board.grid()[5][6] = Cell({5, 6}, tank2);
...
TEST_F(BoardTest, ShellMovesTwoStepsPerTick) {
    ...
    EXPECT_EQ(newPos.first, oldPos.first);
    EXPECT_EQ(newPos.second, oldPos.second + 2);
}
...
TEST_F(BoardTest, WallDestroyedAfterTwoShellHits) {
    ...
    auto& player1 = board.players()[1];
    auto tank = player1.tank();
    tank->position() = std::make_pair(5, 3);
    tank->direction() = Direction::R;
...
TEST_F(BoardTest, TankWrapsAroundBoardEdges) {
    ...
    EXPECT_EQ(tank->position().first, 9);
    EXPECT_EQ(tank->position().second, 0);  // Wrapped around horizontally
}
